...
func main() {
	...
	for !rl.WindowShouldClose() {
		...
		//rl.DrawGrid(50, 1.0)
		//rl.DrawModel(rl.LoadModelFromMesh(rl.GenMeshPoly(6, 10)), rl.NewVector3(float32(screenPoint.X), float32(screenPoint.Y), 0), 1, rl.Green)
		//	rl.EndMode3D()

		secondCamera.Zoom += float32(rl.GetMouseWheelMove()) * 0.05

		if secondCamera.Zoom > 3.0 {
			secondCamera.Zoom = 3.0
		} else if secondCamera.Zoom < 0.1 {
			secondCamera.Zoom = 0.1
		}
		raygui.Label(rl.NewRectangle(500, 330, 200, 20), "TextBox")
		inputText = raygui.TextBox(rl.NewRectangle(500, 350, 200, 20), inputText) // Debug Text
		rl.BeginMode2D(secondCamera)
		rl.DrawPoly(rl.NewVector2(float32(screenPoint.X), float32(screenPoint.Y)), 6, 12, 0, rl.Red)
		...
		rl.DrawText("Camera Z: "+fmt.Sprintf("%.2f", secondCamera.Zoom), 10, 50, 20, rl.Gray)
